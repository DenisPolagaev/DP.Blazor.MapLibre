...
using DP.Blazor.MapLibre.Models.Image;
...
using DP.Blazor.MapLibre.Models.Sprite;
...
public partial class MapLibre : ComponentBase, IAsyncDisposable
{
    ...
    /// If <c>tiles</c>, <c>url</c>, <c>bounds</c>, <c>minzoom</c>, <c>maxzoom</c>, or <c>tileSize</c>
    /// changed, recreates the source with <c>removeLayer</c> / <c>removeSource</c> / <c>addSource</c> / <c>addLayer</c>.
    /// Native <c>setTiles</c> does not invalidate the worker tile cache, so PMTiles timeline
    /// frames would keep showing the first archive.
    /// </summary>
