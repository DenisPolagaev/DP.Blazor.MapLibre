/* Adapted from https://github.com/mapbox/mapbox-gl-framerate (ISC) for MapLibre GL JS */

export function registerFramerateControl(maplibregl) {
    if (!maplibregl) {
        throw new Error('MapLibre GL JS must be loaded before registering the frame rate control.');
    }

    if (maplibregl.FrameRateControl) {
        return maplibregl.FrameRateControl;
    }
    ...
    class FrameRateControl {
        ...
        getFPS = (now) => {
            ...
            return Math.round((1000 * this.frames) / (now - this.time)) || 0;
        };
        ...
        updateGraph = (fpsNow) => {
            ...
            const fps = Math.round((1000 * this.totalFrames) / this.totalTime) || 0;
            const rect = barWidth;
